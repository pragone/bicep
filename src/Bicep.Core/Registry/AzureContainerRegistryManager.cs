...
namespace Bicep.Core.Registry
{
    public class AzureContainerRegistryManager
    {
        ...
        public async Task<OciArtifactResult> PullModuleArtifactsAsync(RootConfiguration configuration, OciArtifactModuleReference moduleReference, bool downloadSource = true)
        {
            ...
            {
                sourcesStream = await PullSourcesAsync(client, moduleReference, mainManifestDigest);
            }

            return new OciArtifactResult(mainManifestDigest, manifest, manifestStream, moduleStream, sourcesStream);
        }

        private async Task<Stream?> PullSourcesAsync(IOciRegistryContentClient client, OciArtifactModuleReference moduleReference, string mainManifestDigest)
        {
        ...
        public async Task PushModuleAsync(RootConfiguration configuration, OciArtifactModuleReference moduleReference, string? artifactType, StreamDescriptor config, Stream? bicepSources, string? documentationUri, string? description, StreamDescriptor[] layers)
        {
            ...
            var moduleManifestDescriptor = await PushModuleManifestAsync(blobClient, configuration, moduleReference, artifactType, config, documentationUri, description, layers, timestamp);
            if (bicepSources is not null)
            {
                await PushSourceManifest(blobClient, moduleReference, moduleManifestDescriptor, bicepSources, timestamp);
            }
        }

        private async Task<OciDescriptor> PushModuleManifestAsync(IOciRegistryContentClient blobClient, RootConfiguration configuration, OciArtifactModuleReference moduleReference, string? artifactType, StreamDescriptor config, string? documentationUri, string? description, StreamDescriptor[] layers, string timestamp)
        {
            ...
            var moduleManifestDescriptor = await PushArtifact(
                blobClient,
                moduleReference.Tag,
                subject: null,
                artifactType,
        ...
        private async Task<OciDescriptor> PushSourceManifest(IOciRegistryContentClient blobClient, OciArtifactModuleReference moduleReference, OciDescriptor moduleManifest, Stream bicepSources, string timestamp)
        {
             ...
             */
            // Azure Container Registries won't recognize this as a valid attachment unless this is valid JSON, so write out "{}" as the configuration content
            ...
            var sourceManifestDescriptor = await PushArtifact(
                blobClient,
                tag: null,
        ...
        private async Task<OciDescriptor> PushArtifact(
            IOciRegistryContentClient blobClient,
            string? tag,
            OciDescriptor? subject, // Specifies a manifest that this artifact "refers" to, thus "attaching" this artifact to the subject manifest. This manifest will remain alive as long as the referred manifest exists.
            string? artifactType,
        ...
        private static async Task<Stream> GetModuleArmTemplateFromManifest(IOciRegistryContentClient client, OciManifest manifest)
        {
            ...
            static void ValidateConfig(OciDescriptor config)
            {
