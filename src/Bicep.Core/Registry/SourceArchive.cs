...
public class SourceArchive : IDisposable
{
    ...
    // Previous versions must be able to deal with unrecognized source kinds.
    ...
    public static Stream PackSources(Uri entrypointFileUri, params ISourceFile[] sourceFiles)
    {
        ...
        using (ZipArchive zipArchive = new ZipArchive(stream, ZipArchiveMode.Create, leaveOpen: true))
        {
            ...
            foreach (var file in sourceFiles)
            {
                ...
                var (location, archivedPath) = GetFilePaths(file.FileUri);
                WriteNewEntry(zipArchive, archivedPath, source);
            ...
            WriteNewEntry(zipArchive, MetadataArchivedFileName, metadataContents);
        }
