...
public static class AzNamespaceType
{
    ...
    private static Lazy<ImmutableArray<(FunctionOverload, ResourceScope)>> ScopeBasedFunctionOverloadsLazy { get; }
        = EnumerableExtensions.CreateLazyArray(GetScopeBasedFunctionOverloads);
    ...
    private static FunctionResult GetDeploymentReturnResult(IBinder binder, IFileResolver fileResolver, IDiagnosticWriter diagnostics, FunctionCallSyntaxBase functionCall, ImmutableArray<TypeSymbol> argumentTypes)
    {
        ...
        if (!binder.TargetScope.HasFlag(ResourceScope.ResourceGroup))
        {
    ...
    private static IEnumerable<(FunctionOverload, ResourceScope)> GetScopeBasedFunctionOverloads()
    {
    ...
    private static IEnumerable<FunctionOverload> GetFunctionOverloads()
    {
        ...
        yield return new FunctionOverloadBuilder("deployment")
            .WithReturnResultBuilder(GetDeploymentReturnResult, LanguageConstants.Object)
    ...
    public static NamespaceType Create(string aliasName, ResourceScope targetScope, AzResourceTypeProvider resourceTypeProvider)
    {
        var functionOverloads = FunctionOverloadsLazy.Value;
        functionOverloads = functionOverloads.AddRange(
            ScopeBasedFunctionOverloadsLazy.Value
                .Where(x => targetScope == (x.Item2 & targetScope))
                .Select(x => x.Item1));
