...
metadata description = 'fake bicep file'";
            ...
            var bicepMain = SourceFileFactory.CreateBicepFile(new Uri("untitled://main.bicep"), mainBicepContents);
            using var zip = SourceBundle.PackSources(fs, bicepMain.FileUri, bicepMain);

            SourceBundle.UnpackSources()
            SourceBundle sourceBundle = new SourceBundle(fs, bundleFolder);
            //SourceBundle sourceBundle = new SourceBundle(fs, bundleFolder);

            SourceBundle.PackSources(fs,)
        }
